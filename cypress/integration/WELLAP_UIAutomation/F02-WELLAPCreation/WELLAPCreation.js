...
And('User verifies Amount on the Invoice page', function () {
    cy.get(this.locator.WellAP.getPaymentAmount).should('contain', '210')
})
...
And('User clicks on pay now button', function () {
    cy.xpath(this.locator.WellAP.Paynowbtn).click({ force: true })
    cy.wait(5000)
})
...
And('Get the WELL AP Eligibility ID and store into Json', function () {
    cy.get(this.locator.WellAP.WELLAPEligibilityID).eq(0).invoke('text').then((ProjId) => {
